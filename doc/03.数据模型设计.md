...
在建立商品模型时，需要用到百度的富文本编辑器UEditor，但是由于官方的Django UEditor停止维护、不再支持Python3，因此需要自行下载，可点击https://download.csdn.net/download/CUFEECR/12641702或https://github.com/twz915/DjangoUeditor3/下载并解压后，将DjangoEeditor目录放入项目的**extra_apps**目录下，并在`settings.py`中添加app如下：
```python
INSTALLED_APPS = [
    'django.contrib.auth',
