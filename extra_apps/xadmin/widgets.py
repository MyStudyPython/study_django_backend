...
class AdminSplitDateTime(forms.SplitDateTimeWidget):
    ...
    def format_output(self, rendered_widgets):
        return mark_safe(
            '<div class="datetime clearfix">%s%s</div>'
...
class AdminRadioInput(RadioChoiceInput):
    def render(self, name=None, value=None, attrs=None, choices=()):
        ...
        if attrs.get("inline", False):
            ...
            return mark_safe(
                '<div class="radio"><label%s>%s %s</label></div>'
                % (label_for, self.tag(), choice_label)
            )
    ...
    def render(self, name, value, attrs=None, choices=()):
        ...
        for i, (option_value, option_label) in enumerate(chain(self.choices, choices)):
            ...
            if final_attrs.get("inline", False):
                output.append(
                    '<label%s class="checkbox-inline">%s %s</label>'
...
class AdminFileWidget(forms.ClearableFileInput):
    template_with_initial = (
        '<p class="file-upload">%s</p>' % forms.ClearableFileInput.initial_text
    )
    template_with_clear = (
        '<span class="clearable-file-input">%s</span>'
