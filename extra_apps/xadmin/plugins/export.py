...
class ExportPlugin(BaseAdminPlugin):
    ...
    def _get_objects(self, context):
        ...
        return [
            dict(
                [
                    (force_str(headers[i].text), self._format_value(o))
    ...
    def _get_datas(self, context):
        ...
        new_rows.insert(
            0, [force_str(c.text) for c in context["result_headers"].cells if c.export]
        )
        return new_rows
    ...
    def get_xlsx_export(self, context):
        ...
        sheet = book.add_worksheet("%s %s" % (_("Sheet"), force_str(model_name)))
        styles = {
    ...
    def get_json_export(self, context):
        ...
        return json.dumps(
            {"objects": results},
            ensure_ascii=False,
            indent=(self.request.GET.get("export_json_format", "off") == "on")
            and 4
            or None,
        )
    ...
    def result_header(self, item, field_name, row):
        item.export = (
            not item.attr
            or field_name == "__str__"
            or getattr(item.attr, "allow_export", True)
        )
        return item
