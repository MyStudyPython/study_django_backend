...
class OrderInfo(models.Model):
    ...
    order_sn = models.CharField(
        max_length=30, unique=True, null=True, blank=True, verbose_name="订单号"
    )
    trade_no = models.CharField(
        max_length=50, unique=True, null=True, blank=True, verbose_name="交易号"
    )
    pay_status = models.CharField(
        max_length=30, default="paying", choices=ORDER_STATUS, verbose_name="订单状态"
    )
    pay_script = models.CharField(
        max_length=11, default="", null=True, blank=True, verbose_name="订单留言"
    )
...
class OrderGoods(models.Model):
    ...
    order = models.ForeignKey(
        OrderInfo, verbose_name="订单信息", on_delete=models.CASCADE, related_name="goods"
    )
    goods = models.ForeignKey(
        Goods, verbose_name="商品", null=True, on_delete=models.SET_NULL
    )
